...
class PreferenceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Module::isEnabled('GeoLocale')) {
            $data['phoneCountries'] = \Modules\GeoLocale\Entities\Country::select('id', 'name', 'code', 'callingcode')
                ->orderBy('name')
                ->get();
        } else {
            $data['phoneCountries'] = collect();
        }
        ...
        if ($request->isMethod('get')) {
            ...
            if (Module::isEnabled('GeoLocale')) {
                $codes = $request->input('phone_country_codes', []);
                if (is_array($codes)) {
                    $codes = array_values(array_map('strtolower', array_filter($codes)));
                    $request['phone_country_codes'] = empty($codes) ? '' : json_encode($codes);
                } else {
                    $request['phone_country_codes'] = is_string($codes) ? $codes : '';
                }
            }
