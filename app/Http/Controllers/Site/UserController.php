...
class UserController extends Controller
{
    ...
    public function activity()
    {
        $lastLogins = DB::table(config('activitylog.table_name'))
            ...
            ->groupBy(['properties->ip_address', 'properties->browser'])
            ->get();

        $idArray = $lastLogins->pluck('id')->toArray();
        ...
        $userActivities->getCollection()->transform(function ($activity) {
            $activity->properties_array = json_decode($activity->properties, true) ?? [];
            return $activity;
        });

        return view('site.myaccount.activities', compact('userActivities'));
    }
}
