...
class ProductAction
{
    ...
    /**
     * Normalize channels to valid values (all possible); default to all channels when empty.
     *
    ...
    {
        $allPossible = ProductChannel::allChannels();
        $channels = array_values(array_intersect($channels, $allPossible));

        return $channels !== [] ? $channels : ProductChannel::allChannels();
    }
    ...
    protected function preProcessRequestArray($array)
    {
        ...
        if (array_key_exists('channels', $array) || ! empty($array['channels_section_saved'])) {
            $activeChannels = ProductChannel::all();
            $allPossibleChannels = ProductChannel::allChannels();
            $channels = isset($array['channels']) && is_array($array['channels']) ? $array['channels'] : [];
            $channels = array_values(array_filter($channels, fn ($c) => in_array($c, $activeChannels, true)));
            if ($channels === []) {
                $array['channels'] = [];
            } else {
                $existingChannels = is_array($this->product->channels ?? null) ? $this->product->channels : [];
                $inactiveChannels = array_values(array_intersect(
                    $existingChannels,
                    array_diff($allPossibleChannels, $activeChannels)
                ));
                $array['channels'] = array_values(array_unique(array_merge($channels, $inactiveChannels)));
            }
        }

        unset($array['channels_section_saved']);
        ...
        if ($rootProduct->isVariableProduct() && $rootProduct->variations) {
            ...
            foreach ($rootProduct->variations as $variation) {
                $product = Product::create([
                    ...
                    'manage_stocks' => $variation->manage_stocks ?? 0,
                    'total_stocks' => $variation->total_stocks ?? 0,
                    'type' => $variation->type,
                    'channels' => $this->normalizeChannels($variation->channels ?? []),
