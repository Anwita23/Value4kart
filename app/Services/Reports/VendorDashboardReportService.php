...
class VendorDashboardReportService
{
    ...
    /**
     * Vendor stats for the current vendor (orders, sales, etc.)
     *
     * @param  string|null  $key
     * @return DashboardReportService
     */
    public function vendorStats($key = 'vendorStats')
    {
        $vendorId = $this->getVendorId();
        $thisMonthOrders = OrderDetail::join('orders', 'orders.id', 'order_details.order_id')
            ->where('order_details.vendor_id', $vendorId)
            ->where('orders.order_date', '>=', $this->offsetDate('-30'))
            ->groupBy('orders.id')
            ->get()
            ->count();
        $thisMonthSales = OrderDetail::where('order_details.vendor_id', $vendorId)
            ->leftJoin('orders', 'orders.id', '=', 'order_details.order_id')
            ->where('orders.order_date', '>=', $this->offsetDate('-30'))
            ->where('orders.payment_status', 'Paid')
            ->sum(DB::raw('order_details.price * order_details.quantity'));
