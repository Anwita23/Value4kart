...
class OrderStatus extends Model
{
    ...
    /**
     * Get only Paid and Unpaid order statuses for invoice create.
     * Paid uses Completed; Unpaid uses the first unpaid status by order.
     *
     * @return \Illuminate\Support\Collection<int, OrderStatus>
     */
    public static function getPaidAndUnpaidForInvoice(): \Illuminate\Support\Collection
    {
        $unpaid = self::getAll()->where('payment_scenario', 'unpaid')->sortBy('order_by')->first();
        $paid = self::getAll()->where('slug', 'completed')->first();

        return collect([$unpaid, $paid])->filter();
    }
}
