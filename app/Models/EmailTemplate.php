...
class EmailTemplate extends Model
{
    ...
    protected static function storeValidation($data = [])
    {
        ...
        $validator = Validator::make($data, [
            ...
            'slug' => [
                'required',
                Rule::unique('email_templates', 'slug')->where(function ($query) use ($parentId) {
                    if ($parentId === null) {
     ...
     * @return mixed
     */
    protected static function updateValidation($data = [], $id = null, $exist = null)
    {
        $parentId = $data['parent_id'] ?? null;
        if ($parentId === null && $id !== null) {
            $parentId = $exist ? $exist->parent_id : optional(static::find($id))->parent_id;
        }
        $validator = Validator::make($data, [
            'name' => 'required|max:50',
            'slug' => [
