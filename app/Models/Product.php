...
class Product extends Model implements BatchDeletable
{
    ...
    protected $fillable = [
        ...
        'sale_from', 'sale_to', 'featured', 'type', 'channels', 'manage_stocks', 'total_stocks', 'menu_order',
    ];
    ...
    protected static function booted()
    {
        ...
        static::creating(function ($product) {
            ...
            if (! isset($product->channels) || $product->channels === null) {
                $product->channels = [];
            }
        });
    }
     ...
     * @return collection
     */
    public static function bestSeller($limit = null, $data = [])
    {
        return self::getProductCategorizer($data ?? [])
     ...
     * @return collection
     */
    public static function popularProducts($limit = null, $data = [])
    {
        return self::getProductCategorizer($data ?? [])
            ->setLimit(self::getLimit($limit))
            ->popularProducts()->get();
     ...
     * @return collection
     */
    public static function featureProducts($limit = null, $data = [])
    {
        return self::getProductCategorizer($data ?? [])
            ->setLimit(self::getLimit($limit))
            ->featureProducts()->get();
     ...
     * @return collection
     */
    public static function newArrivals($limit = null, $data = [])
    {
        return self::getProductCategorizer($data ?? [])
            ->setLimit(self::getLimit($limit))
            ->newArrivals()->get();
     ...
     * @return collection
     */
    public static function bestDeals($limit = null, $data = [])
    {
        return self::getProductCategorizer($data ?? [])
            ->setLimit(self::getLimit($limit))
            ->bestDeals()->get();
     ...
     * @return collection
     */
    public static function flashSales($limit = null, $data = [])
    {
        return self::getProductCategorizer($data ?? [])
            ->setLimit(self::getLimit($limit))
            ->flashSales()->get();
     ...
     *
     * @param  array  $data  Optional: 'channel' (e.g. ProductChannel::$Store, $Web), 'vendor_id'
     * @return ProductCategorizingService
     */
    public static function getProductCategorizer(array $data = [])
    {
        $query = self::query()->filter(HomePageProductFilter::class);
        if (array_key_exists('channel', $data)) {
            $query->whereJsonContains('channels', $data['channel']);
        }
        if (array_key_exists('vendor_id', $data) && $data['vendor_id'] !== null && $data['vendor_id'] !== '') {
            $query->where('vendor_id', $data['vendor_id']);
        }

        return new ProductCategorizingService($query, false);
    }
