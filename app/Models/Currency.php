...
class Currency extends Model
{
    ...
    public static function getDefault()
    {
        if (self::$defaultCurrency === null) {
            self::$defaultCurrency = self::where('id', preference('dflt_currency_id'))->first();

            if (empty(self::$defaultCurrency)) {
                self::$defaultCurrency = self::first();
                if (self::$defaultCurrency) {
                    Preference::where('field', 'dflt_currency_id')
                        ->update(['field' => 'dflt_currency_id', 'value' => self::$defaultCurrency->id]);
                }
            }
        }
