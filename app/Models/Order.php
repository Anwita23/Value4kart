...
class Order extends Model
{
    ...
    public function remove($id = null)
    {
        ...
        if (! empty($record)) {
            try {
                ...
                self::adjustStockWhenOrderDeleted($id);
                $record->delete();
                $data['status'] = 'success';
    ...
    /**
     * Adjust stock when order deleted
     *
     * @param  int  $orderId
     */
    public static function adjustStockWhenOrderDeleted($orderId): void
    {
        $order = parent::find($orderId);
        $locationId = $order->location_id ?? ($order->getMeta()['location_id'] ?? null);
        $orderStatus = OrderStatus::getAll()->where('id', $order->order_status_id)->first();
        if (! $locationId || $orderStatus?->slug == 'cancelled') {
            return;
        }
    ...
    public function invoicePdfEmail($order = null, $invoiceName = null, $type = 'admin', $vendorId = null)
    {
        if (! empty($order)) {
            ...
            if ($type == 'admin' && $order->channel == 'web') {
                ...
                $vendorId ??= $order->orderDetails->first()?->vendor_id;
