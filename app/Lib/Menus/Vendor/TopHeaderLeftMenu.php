    ...
    public static function get(): array
    {
        ...
        $items = [
            ...
            'admin_panel' => [
                ...
                'position' => '40',
                'visibility' => (optional($user->role())->type == 'admin'),
            ],
            'shop' => [
                'item' => (function () use ($user) {
                    $shop = $user->shop();
                    $alias = $shop && ! empty($shop->alias) ? $shop->alias : null;

                    return $alias
                        ? '<a class="d-flex align-items-center text-decoration-none" href="' . route('site.shop', ['alias' => $alias]) . '" target="_blank">
                        <i class="feather icon-external-link"></i><span class="ltr:ms-2 rtl:me-2 list-curent-color">' . __('Shop') . '</span></a>'
                        : '';
                })(),
                'position' => '40',
                'visibility' => ! empty($user->shop()) && isset(optional($user->shop())->alias) && (optional($user->role())->type == 'vendor' || optional($user->role())->type == 'admin'),
            ],
        ];
