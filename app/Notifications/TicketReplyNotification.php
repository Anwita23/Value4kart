...
class TicketReplyNotification extends Notification
{
    ...
    public function replaceSmsVariables($body)
    {
        ...
        $isVendor = $assignData->vendor();
        $ticket_reply_url = $isVendor ? 'vendor/ticket/reply/' . base64_encode($this->request['emailInfo']->id) : 'admin/ticket/reply/' . base64_encode($this->request['emailInfo']->id);
        $ticket_reply = url('admin/ticket/reply/' . base64_encode($this->request['emailInfo']->id));

        $data = [
            '{name}' => $assignData->name,
            '{ticket_message}' => strip_tags(optional($this->request['emailInfo']->threadReplies[0])['message']),
            '{tickets_link}' => url($ticket_reply_url),
