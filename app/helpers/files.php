...
if (! function_exists('domPdfImageSource')) {
     ...
     *
     * @param  string  $url  Full URL or path to image (normalized to forward slashes for URLs)
     * @param  string  $type  MIME type, e.g. image/png
     * @return string  Data URI (base64) or a minimal transparent 1x1 PNG on failure
     */
    function domPdfImageSource($url, $type = 'image/png')
    {
        if (empty($url) || ! is_string($url)) {
            return domPdfImageSourceFallback($type);
        }
        ...
        if (isset($parsed['path'])) {
            ...
            if (str_starts_with($path, '/public/')) {
                $localPath = public_path(substr($path, 8));
            } elseif (str_starts_with($path, '/storage/')) {
                $localPath = storage_path('app/public' . substr($path, 8));
            } elseif (! str_contains($url, '://') && is_file($url)) {
                $localPath = $url;
            }
        }
        ...
        try {
            if ($localPath && is_file($localPath)) {
                return 'data:' . $type . ';base64,' . base64_encode(file_get_contents($localPath));
            }
...
if (! function_exists('domPdfImageSourceFallback')) {
    /**
     * Fallback data URI (1x1 transparent PNG) when image cannot be loaded for dompdf.
     */
    function domPdfImageSourceFallback(string $type = 'image/png'): string
    {
        $transparentPng = base64_decode('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==', true);

        return 'data:' . $type . ';base64,' . base64_encode($transparentPng);
    }
}
