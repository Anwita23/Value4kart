...
/**
 * Merge dial_code and phone into a single phone value (full international number).
 * Call before validation or saving whenever a form may submit dial_code + phone.
 *
 * @param  \Illuminate\Http\Request|array  $requestOrArray
 * @return \Illuminate\Http\Request|array
 */
function merge_dial_code_into_phone($requestOrArray)
{
    $isRequest = $requestOrArray instanceof \Illuminate\Http\Request;
    $dialCode = $isRequest ? $requestOrArray->input('dial_code') : ($requestOrArray['dial_code'] ?? null);
    $phone = $isRequest ? $requestOrArray->input('phone') : ($requestOrArray['phone'] ?? null);

    if (filled($dialCode) && filled($phone)) {
        $merged = $dialCode . $phone;
        if ($isRequest) {
            $requestOrArray->merge(['phone' => $merged]);

            return $requestOrArray;
        }
        $requestOrArray['phone'] = $merged;
...
{
    // Disable recaptcha in testing so login/registration/password-reset tests do not require g-recaptcha-response
    if (app()->runningUnitTests()) {
        return false;
    }

    return isActive('Recaptcha') && ! empty(env('NOCAPTCHA_SITEKEY')) && ! empty(env('NOCAPTCHA_SECRET'));
}
