...
@section('content')
    <div class="list-container" id="vendor-refund-edit-container">
                                    ...
                                    <form action="{{ route('vendor.refund.update', ['id' => $refund->id]) }}" method="post" class="form-horizontal p-2" id="delete-refund-{{ $refund->id }}">
                                        ...
                                        <div class="col-sm-8 px-0">
                                            <div class="w-100 admin-form-actions">
                                                <x-backend.button.cancel :href="route('vendor.refund.index')" :label="__('Cancel')" class="all-cancel-btn" />
                                                @if (!preg_match('/^Accepted$|^Declined$/', $refund->status))
                                                    <x-backend.button.save type="button" id="submitBtn" :label="__('Update')"
                                                        data-id="{{ $refund->id }}"
                                        ...
                                        @if (in_array($refund->status, ['Opened', 'In progress']))
                                            <div class="ml-50p w-100 mt-2">
                                                <form action="{{ route('vendor.refundProcess') }}" method="post">
                                                    ...
                                                    <div class="flex">
                                                        <x-backend.button.save type="submit" :label="__('Send')" class="w-100" />
                                                    </div>
                                                </form>
                                            </div>
                                        @endif
        ...
        <x-backend.delete-modal :includeScript="false" />
    </div>
@endsection
