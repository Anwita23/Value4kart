...
<x-backend.button.cancel :href="isset($_GET['vendor_id']) ? route('vendors.edit', ['id' => $_GET['vendor_id']]) : route('vendor.shop.index')" :label="__('Cancel')" />
                  <x-backend.button.save type="submit" id="submitBtn" :label="''">
                      <i class="comment_spinner spinner fa fa-spinner fa-spin custom-btn-small display_none"></i><span id="spinnerText">{{ __('Submit') }}</span>
