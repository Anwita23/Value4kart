...
class MenuController extends Controller
{
    ...
    /**
     * Resolve a page URL or path to the matching route name (for "active menu" setup).
     * Lets non-technical users paste a URL from their browser instead of typing route names.
     *
     * @param  Request  $request  expects 'path' or 'url' (e.g. /admin/products or full URL)
     * @return \Illuminate\Http\JsonResponse
     */
    public function resolveRouteFromUrl(Request $request)
    {
        $path = $request->input('path') ?: $request->input('url');
        if (empty($path) || ! is_string($path)) {
            return response()->json(['error' => __('Please provide a page URL or path.')], 422);
        }
        ...
        // Strip application base path (e.g. /martvill) so the path we pass to the router
        // is the route URI (e.g. /admin/categories). Request::create() does not set base path,
        // so if we used full URL the router would see "martvill/admin/categories" and fail.
        $basePath = $request->getBasePath();
        if ($basePath !== '' && strpos($path, $basePath) === 0) {
            $pathForMatch = substr($path, strlen($basePath)) ?: '/';
        } else {
            $pathForMatch = $path;
        }
        $pathForMatch = '/' . ltrim($pathForMatch, '/');

        try {
            // Create a URI whose path is only the route path (e.g. /admin/categories). If we used
            // root() + pathForMatch we'd get .../martvill/admin/categories and the router would
            // see path "martvill/admin/categories" and fail. So use scheme+host+pathForMatch.
            $uri = $request->getSchemeAndHttpHost() . $pathForMatch;
            $fakeRequest = Request::create($uri, 'GET');
            $route = Route::getRoutes()->match($fakeRequest);
            $name = $route->getName();

            return $name
                ? response()->json(['route_name' => $name, 'path' => $path])
                : response()->json(['error' => __('This page has no named route.')], 404);
        } catch (\Throwable $e) {
            return response()->json(['error' => __('No matching route found for this URL.')], 404);
        }
    }
}
