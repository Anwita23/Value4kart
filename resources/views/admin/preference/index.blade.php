                    ...
                    <div class="card-block table-border-style">
                        <form action="{{ route('preferences.index') }}" method="post" class="form-horizontal" id="preference_form">
                            ...
                            <div class="card-body p-0">
                                ...
                                @if(isset($phoneCountries) && $phoneCountries->isNotEmpty())
                                <div class="form-group row">
                                    <label class="col-sm-3 control-label text-left"
                                        for="phone_country_codes">{{ __('Phone Dial Code Countries') }}</label>
                                    <div class="col-sm-6">
                                        <select class="form-control select2-hide-search" name="phone_country_codes[]" id="phone_country_codes" multiple>
                                            @foreach($phoneCountries as $country)
                                                <option value="{{ strtolower($country->code) }}"
                                                    {{ in_array(strtolower($country->code), array_map('strtolower', (array) json_decode(preference('phone_country_codes'), true) ?? [])) ? 'selected' : '' }}>
                                                    {{ $country->name }} (+{{ $country->callingcode ?? '' }})
                                                </option>
                                            @endforeach
                                        </select>
                                        <small class="form-text text-muted">{{ __('Leave empty to show all country codes in phone fields.') }}</small>
                                    </div>
                                </div>
                                @endif
                                <div class="form-group row">
