            ...
            <div class="flex flex-row justify-between mt-19p">
                <div class="flex flex-col">
                    <p class="dm-sans text-gray-12 text-sm">
                        @if (optional($row['product'])->slug)
                            <a href="{{ route('site.productDetails', ['slug' => $row['product']->slug]) }}">{{ $row['item']->product_name }}</a>
                        @else
                            <span>{{ $row['item']->product_name }}</span>
                        @endif
                        <span class="px-4">x</span>
                        <span>{{ (int) $row['item']->quantity }}</span>
                    </p>
                    <p class="text-gray-10 roboto-medium text-xs mt-3p">
                        @if (!is_null(optional($row['vendor'])->name))
                            <span>{{ __('Vendor') }}</span>
                            <span class="px-1">:</span>
                            <a href="{{ route('site.shop', ['alias' => $row['vendor']->shops->first()->alias]) }}" class="text-gray-600">{{ $row['vendor']->name }}</a>
                        @endif
                    </p>
                </div>
                <div class="right-side">
                    <span class="dm-sans text-gray-12 text-sm">{{ formatNumber($row['item']->price * $row['item']->quantity) }}</span>
                </div>
            </div>
        @endforeach
    </div>
